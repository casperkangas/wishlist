...
struct SidebarView: View {
    ...
    private var sortedCategories: [WishCategory] {
        ...
        let nonEmpty = store.categories
            ...
            .sorted { a, b in
                let latestA = liveItems.filter { $0.categoryID == a.id }.map(\.createdAt).max() ?? .distantPast
                let latestB = liveItems.filter { $0.categoryID == b.id }.map(\.createdAt).max() ?? .distantPast
